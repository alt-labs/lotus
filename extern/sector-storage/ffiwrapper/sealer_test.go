...
func (s *seal) commit(t *testing.T, sb *Sealer, done func()) storage.Proof {
	...
	ok, err := ProofVerifier.VerifySeal(minertypes.SealVerifyInfo{
		SectorID:              s.ref.ID,
...
func post(t *testing.T, sealer *Sealer, skipped []abi.SectorID, seals ...seal) {
	...
	xsis := make([]minertypes.ExtendedSectorInfo, len(seals))
	for i, s := range seals {
		xsis[i] = minertypes.ExtendedSectorInfo{
			SealProof:    s.ref.ProofType,
	...
	sis := make([]minertypes.SectorInfo, len(seals))
	for i, xsi := range xsis {
		sis[i] = minertypes.SectorInfo{
	...
	ok, err := ProofVerifier.VerifyWindowPoSt(context.TODO(), minertypes.WindowPoStVerifyInfo{
...
func TestSealAndVerifyAggregate(t *testing.T) {
	...
	avi := minertypes.AggregateSealVerifyProofAndInfos{
		Miner:          miner,
		SealProof:      sealProofType,
		AggregateProof: policy.GetDefaultAggregationProof(),
		Proof:          nil,
		Infos:          make([]minertypes.AggregateSealVerifyInfo, numAgg),
	}
	...
	for i := 0; i < numAgg; i++ {
		...
		avi.Infos[i] = minertypes.AggregateSealVerifyInfo{
